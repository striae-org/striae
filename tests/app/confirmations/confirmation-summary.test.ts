...
import type { UserConfirmationSummaryDocument, CaseConfirmationSummary } from '~/utils/data/confirmation-summary/summary-core';
...
function makeSummaryDocument(cases: Record<string, CaseConfirmationSummary>): UserConfirmationSummaryDocument {
	return {
		version: 1,
		updatedAt: new Date().toISOString(),
		cases,
...
describe('removeFileConfirmationSummary', () => {
	...
	it('is a no-op when the file does not exist in the summary', async () => {
		...
		await expect(removeFileConfirmationSummary(mockUser, 'CASE-001', 'nonexistent-file')).resolves.not.toThrow();
...
describe('removeCaseConfirmationSummary', () => {
	...
	it('is a no-op when the case does not exist', async () => {
		...
		await expect(removeCaseConfirmationSummary(mockUser, 'NONEXISTENT-CASE')).resolves.not.toThrow();
