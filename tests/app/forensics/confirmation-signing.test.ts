...
import { FORENSIC_MANIFEST_SIGNATURE_ALGORITHM } from '~/utils/forensics/SHA256';
...
async function signWithKey(payload: string, privateKey: CryptoKey): Promise<string> {
	const sig = await crypto.subtle.sign({ name: 'RSA-PSS', saltLength: 32 }, privateKey, new TextEncoder().encode(payload));
...
describe('verifyConfirmationSignature', () => {
	...
	async function buildSignedConfirmation(data: ConfirmationImportData): Promise<ConfirmationImportData> {
