...
async function decryptAesGcm(ciphertext: Uint8Array, key: CryptoKey, iv: Uint8Array): Promise<string> {
...
describe('generateSharedAesKey', () => {
	...
	it('generates a unique key each call', async () => {
		...
		const [raw1, raw2] = await Promise.all([crypto.subtle.exportKey('raw', k1), crypto.subtle.exportKey('raw', k2)]);
