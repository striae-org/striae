...
import { signPayload, type WorkerSignatureEnvelope } from '../../../workers/data-worker/src/signature-utils';
...
async function verifyRsaPss(payload: string, signatureBase64url: string, publicKeyPem: string): Promise<boolean> {
	...
	const key = await crypto.subtle.importKey('spki', spkiBytes.buffer, { name: 'RSA-PSS', hash: 'SHA-256' }, false, ['verify']);
...
describe.each([2048, 3072])('signPayload (modulusLength=%i)', (modulusLength) => {
	...
	it('produces a signature verifiable with the corresponding public key', async () => {
		...
		const envelope: WorkerSignatureEnvelope = await signPayload(payload, privateKeyPem, TEST_KEY_ID, TEST_ALGORITHM);
	...
	it('throws when privateKey is undefined', async () => {
		await expect(signPayload('payload', undefined, TEST_KEY_ID, TEST_ALGORITHM)).rejects.toThrow();
	});

	it('throws when keyId is undefined', async () => {
		await expect(signPayload('payload', privateKeyPem, undefined, TEST_ALGORITHM)).rejects.toThrow();
	});

	it('uses the custom error message when provided', async () => {
		await expect(signPayload('payload', undefined, undefined, TEST_ALGORITHM, 'Custom error: secrets not set')).rejects.toThrow(
			'Custom error: secrets not set',
		);
	});
});
