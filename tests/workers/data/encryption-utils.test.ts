...
describe('encryptJsonForStorage / decryptJsonFromStorage', () => {
	...
	it('throws when decrypting with the wrong private key', async () => {
		...
		await expect(decryptJsonFromStorage(ciphertext.buffer as ArrayBuffer, envelope, wrongPrivatePem)).rejects.toThrow();
	});
});
...
describe('decryptExportData', () => {
	...
	async function simulateExportEncrypt(
		...
		const ciphertext = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aesKey, new TextEncoder().encode(plaintext)));
