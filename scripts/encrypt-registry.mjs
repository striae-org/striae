...
function encryptRegistry(plaintextJson, keyBase64) {
	...
	const keyBuffer = Buffer.from(keyBase64.replace(/-/g, '+').replace(/_/g, '/'), 'base64');

	if (keyBuffer.length !== 32) {
		throw new Error(`REGISTRY_ENCRYPTION_KEY must decode to 32 bytes, got ${keyBuffer.length}`);
	}
