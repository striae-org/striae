...
function generateThirdPartyLicenses() {
	...
	if (!fs.existsSync(rootPackageLockPath)) {
		throw new Error('package-lock.json not found. A committed lockfile is required to reproduce the locked production dependency tree.');
	}

	const rootPackageId = rootPackage.version ? `${rootPackage.name}@${rootPackage.version}` : rootPackage.name;
	...
	try {
		...
		fs.writeFileSync(path.join(toolingDir, 'package.json'), `${JSON.stringify(toolingPackageJson, null, 2)}\n`);

		console.log(`Installing license scanner in ${toolingDir}`);
		runNpm(['install', '--no-audit', '--no-fund', '--ignore-scripts'], toolingDir);

		const licenseCheckerCli = path.join(toolingDir, 'node_modules', 'license-checker', 'bin', 'license-checker');

		const scanResult = runCommand(process.execPath, [licenseCheckerCli, '--start', reproDir, '--production', '--json'], {
			cwd: reproDir,
			capture: true,
		});
		...
		const firebaseSharedReadme = path.join(reproDir, 'node_modules', '@firebase', 'app', 'README.md');
		...
		for (const entry of entries) {
			lines.push(
				`| ${toTableCell(entry.name)} | ${toTableCell(entry.version)} | ${toTableCell(entry.license)} | ${toTableCell(entry.repository)} |`,
			);
		}
