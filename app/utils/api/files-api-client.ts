...
export async function fetchFilesApi(user: User, path: string, init: RequestInit = {}): Promise<Response> {
...
export async function createSignedFileUrlApi(user: User, fileId: string, expiresInSeconds?: number): Promise<SignedFileUrlResponse> {
...
export async function uploadOtherFileApi(user: User, file: File, onProgress?: (progress: number) => void): Promise<FileUploadResponse> {
	const userWithOptionalToken = user as User & { getIdToken?: () => Promise<string> };
	...
	const proxyUploadResult = await uploadWithXhr(FILES_API_BASE, `Bearer ${idToken}`, file, onProgress);
