...
async function saveConfirmationSummaryDocument(user: User, summary: UserConfirmationSummaryDocument): Promise<void> {
...
async function getFileAnnotationsForSummary(user: User, caseNumber: string, fileId: string): Promise<AnnotationData | null> {
	try {
...
export const getConfirmationSummaryDocument = async (user: User): Promise<UserConfirmationSummaryDocument> => {
...
export const getCaseConfirmationSummary = async (user: User, caseNumber: string): Promise<CaseConfirmationSummary | null> => {
...
export const ensureCaseConfirmationSummary = async (
	...
	const summary = options.prefetchedSummary ? structuredClone(options.prefetchedSummary) : await getConfirmationSummaryDocument(user);
	...
	const maxAgeMs =
		typeof options.maxAgeMs === 'number' && Number.isFinite(options.maxAgeMs) && options.maxAgeMs > 0
			? options.maxAgeMs
			: DEFAULT_CONFIRMATION_SUMMARY_MAX_AGE_MS;
	const caseIsStale = options.forceRefresh === true || !existingCase || isStaleTimestamp(existingCase.updatedAt, maxAgeMs);
...
export const removeFileConfirmationSummary = async (user: User, caseNumber: string, fileId: string): Promise<void> => {
	const summary = await getConfirmationSummaryDocument(user);
	const caseSummary = summary.cases[caseNumber];
...
export const removeCaseConfirmationSummary = async (user: User, caseNumber: string): Promise<void> => {
	const summary = await getConfirmationSummaryDocument(user);
...
export const moveCaseConfirmationSummary = async (user: User, fromCaseNumber: string, toCaseNumber: string): Promise<void> => {
