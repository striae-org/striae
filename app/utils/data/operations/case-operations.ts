 ...
 */
export const getCaseData = async (user: User, caseNumber: string, options: DataOperationOptions = {}): Promise<CaseData | null> => {
	try {
 ...
 */
export const deleteCaseData = async (user: User, caseNumber: string, options: DataOperationOptions = {}): Promise<void> => {
	try {
		const sessionValidation = await validateUserSession(user);
		if (!sessionValidation.valid) {
			throw new Error(`Session validation failed: ${sessionValidation.reason}`);
 ...
 */
export const caseExists = async (user: User, caseNumber: string): Promise<boolean> => {
	try {
