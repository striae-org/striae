...
export type CasesModalConfirmationFilter = 'all' | 'pending' | 'confirmed' | 'none-requested';
...
export function sortCasesForModal(cases: CasesModalCaseItem[], sortBy: CasesModalSortBy): CasesModalCaseItem[] {
	const next = [...cases];
	...
	return next.sort((left, right) => compareCaseNumbersAlphabetically(left.caseNumber, right.caseNumber));
}
...
export function getCasesForModal(
	...
	return sortCasesForModal(filterCasesForModal(cases, preferences, caseConfirmationStatus), preferences.sortBy);
}
