...
import { type AuditExportSigningPayload, verifyAuditExportSignature } from './audit-export-signature';
...
function getSignatureFailureMessage(error: string | undefined, targetLabel: 'export ZIP' | 'confirmation file' | 'audit export'): string {
...
async function verifyBundledAuditExport(zip: {
	...
	if (!auditTrailContent || !auditSignatureContent) {
		return createVerificationResult(false, 'The archive ZIP contains incomplete bundled audit verification files.', 'case-zip');
	}
	...
	try {
		...
		if (!metadata?.signature || typeof metadata.hash !== 'string') {
			return createVerificationResult(false, 'The bundled audit export is missing required hash or signature metadata.', 'case-zip');
		}

		const unsignedAuditExport =
			auditTrailExport.auditTrail !== undefined
				? {
						metadata: {
		...
		if (recalculatedHash.toUpperCase() !== metadata.hash.toUpperCase()) {
			return createVerificationResult(false, 'The bundled audit export failed integrity verification.', 'case-zip');
		}
		...
		const signatureVerification = await verifyAuditExportSignature(embeddedSignaturePayload, metadata.signature);

		if (!signatureVerification.isValid) {
			return createVerificationResult(false, getSignatureFailureMessage(signatureVerification.error, 'export ZIP'), 'case-zip');
		}
		...
		if (
			...
			return createVerificationResult(false, 'The bundled audit signature artifact does not match the signed audit export.', 'case-zip');
		}

		return null;
	} catch {
		return createVerificationResult(false, 'The bundled audit export could not be parsed for verification.', 'case-zip');
	}
}
...
export async function verifyCasePackageIntegrity(input: CasePackageIntegrityInput): Promise<CasePackageIntegrityResult> {
	...
	const signatureResult = await verifyForensicManifestSignature(input.forensicManifest);

	const integrityResult = await validateCaseIntegritySecure(input.cleanedContent, input.imageFiles, manifestData);

	const bundledAuditVerification = input.bundledAuditFiles
		? await verifyBundledAuditExport({
				file: (path: string) => {
					const content =
						path === 'audit/case-audit-trail.json'
