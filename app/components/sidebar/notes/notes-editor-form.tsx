...
import {
	type AnnotationData,
	type BulletAnnotationData,
	type CartridgeCaseAnnotationData,
	type ShotshellAnnotationData,
	type ItemType,
	type SupportLevel,
	type IndexType,
...
export const NotesEditorForm = ({
	...
	const setSelectedItemData = useCallback(
		(newData: {
			itemType?: ItemType | '';
			customClass?: string;
			classNote?: string;
	...
	const handleSave = useCallback(async (): Promise<boolean> => {
		...
		try {
			...
			const annotationData: AnnotationData = {
				...
				earliestAnnotationTimestamp: resolveEarliestAnnotationTimestamp(undefined, existingData?.earliestAnnotationTimestamp, now),
			};
	...
	return (
		<div className={`${styles.notesEditorForm} ${styles.editorLayout}`}>
			{isLoading ? (
				...
				<>
					{isConfirmedImage && <div className={styles.immutableNotice}>This image is confirmed. Notes are read-only.</div>}
						...
						{isCaseInfoOpen && (
							<>
								...
								<div className={styles.fontColorRow}>
									...
									<ColorSelector selectedColor={caseFontColor} onColorSelect={setCaseFontColor} disabled={areEditsDisabled} />
								</div>
							</>
						)}
					</div>
								...
								<>
									...
									<div className={styles.classCharacteristicsColumns}>
										...
										<div className={styles.itemDetailsPanel}>
											<button type="button" onClick={() => setIsClassDetailsOpen(true)} className={styles.itemDetailsButton}>
							...
							{isIndexOpen && (
								<div className={styles.indexing}>
									...
									{indexType === 'number' ? (
										...
										<ColorSelector selectedColor={indexColor} onColorSelect={setIndexColor} disabled={areEditsDisabled} />
									) : null}
								</div>
							)}
						</div>
					...
					<div className={`${styles.notesActionBar} ${styles.notesActionBarSticky}`}>
						<button
							...
							title={
								isConfirmedImage
									? 'Cannot save notes - image is confirmed'
									: isUploading
										? 'Cannot save notes while uploading'
										: isReadOnly
											? 'Cannot save notes - case is read-only'
											: undefined
							}
						>
							Save Notes
