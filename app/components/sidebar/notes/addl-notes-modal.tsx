...
export const AddlNotesModal = ({ isOpen, onClose, notes, onSave, isReadOnly = false, showNotification }: AddlNotesModalProps) => {
	...
	return (
		<div className={styles.modalOverlay} aria-label="Close notes dialog" {...overlayProps}>
			<div className={styles.modal}>
				...
				<div className={styles.modalButtons}>
					<button onClick={handleSave} className={styles.saveButton} disabled={isSaving || isReadOnly} aria-busy={isSaving}>
						{isSaving ? 'Saving...' : 'Save'}
					</button>
					<button onClick={requestClose} className={styles.cancelButton} disabled={isSaving}>
						Cancel
					</button>
				</div>
			</div>
