...
export const SHOTSHELL_GAUGES: string[] = ['10 gauge', '12 gauge', '16 gauge', '20 gauge', '28 gauge', '.410 bore'];

export const SHOTSHELL_LENGTH_OPTIONS: string[] = ['2 1/2 inch', '2 3/4 inch', '3 inch', '3 1/2 inch'];
...
export const handleSelectWithCustom = (value: string, setValue: (nextValue: string) => void, setIsCustom: (nextValue: boolean) => void) => {
	if (value === CUSTOM) {
		setIsCustom(true);
		setValue('');
		return;
...
export const buildItemDetailsSummary = (
	...
	const r = (label: string, value: string | number | undefined): string | null => (value ? `${label}: ${value}` : null);
...
export const calculateBulletDiameter = (lgCount: number, lWidths: string[], gWidths: string[]): number | null => {
