...
const renderCheckboxes = (items: CheckboxConfig[], isReadOnly: boolean) =>
	items.map((item) => (
		<CheckboxField key={item.key} label={item.label} checked={item.checked} onChange={item.onChange} disabled={isReadOnly} />
	));

export const BulletSection = ({ showHeader, isReadOnly, bullet }: BulletSectionProps) => {
	...
	return (
		<div className={styles.itemDetailsSection}>
			...
			<div className={styles.itemDetailsFieldGrid}>{bulletFields.map((field) => renderConfiguredField(field, isReadOnly))}</div>
			{bullet.lgCount > 0 && (
							...
							{showCalcExplanation && (
								<div className={styles.calcExplanationPanel}>
									...
									<ul className={styles.calcExplanationList}>
										<li>
											<strong>L&#772;</strong> — average land width
										</li>
										<li>
											<strong>G&#772;</strong> — average groove width
										</li>
										<li>
											<strong>n</strong> — L/G count
										</li>
										<li>
											<strong>&pi;</strong> — 3.14159&hellip;
										</li>
									</ul>
									<p className={styles.calcExplanationNote}>
										The bullet&rsquo;s circumference approximates the sum of all land and groove widths. Dividing by &pi; converts
...
export const CartridgeCaseSection = ({ showHeader, isReadOnly, cartridgeCase }: CartridgeSectionProps) => {
	...
	return (
		<div className={styles.itemDetailsSection}>
			...
			<div className={styles.itemDetailsFieldGrid}>{cartridgeFields.map((field) => renderConfiguredField(field, isReadOnly))}</div>
				...
				{renderCheckboxes(
					[
						{ key: 'fpDrag', label: 'FP Drag', checked: cartridgeCase.hasFpDrag, onChange: cartridgeCase.setHasFpDrag },
						{
							...
							checked: cartridgeCase.hasExtractorMarks,
							onChange: cartridgeCase.setHasExtractorMarks,
						},
...
export const ShotshellSection = ({ showHeader, isReadOnly, shotshell }: ShotshellSectionProps) => {
	...
	return (
		<div className={styles.itemDetailsSection}>
			...
			<div className={styles.itemDetailsFieldGrid}>{shotshellFields.map((field) => renderConfiguredField(field, isReadOnly))}</div>
			<div className={styles.itemDetailsCheckboxGroup}>
				{renderCheckboxes(
					[
