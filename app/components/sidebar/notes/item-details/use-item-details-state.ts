...
export const useItemDetailsState = ({ bulletData, cartridgeCaseData, shotshellData }: UseItemDetailsStateParams) => {
	...
	const [bJacketMetalIsCustom, setBJacketMetalIsCustom] = useState(() =>
		isCustomValue(bulletData?.jacketMetal, BULLET_JACKET_METAL_OPTIONS),
	);
	...
	const [cPrimerTypeIsCustom, setCPrimerTypeIsCustom] = useState(() =>
		isCustomValue(cartridgeCaseData?.primerType, CARTRIDGE_PRIMER_TYPE_OPTIONS),
	);
	...
	const [cApertureShapeIsCustom, setCApertureShapeIsCustom] = useState(() =>
	...
	const [sShotshellLengthIsCustom, setSShotshellLengthIsCustom] = useState(() =>
		isCustomValue(shotshellData?.shotshellLength, SHOTSHELL_LENGTH_OPTIONS),
	);
	...
	const buildSaveData = ({ showBullet, showCartridge, showShotshell }: BuildSaveDataParams): BuildSaveDataResult => ({
