...
export const DeleteFilesModal = ({ isOpen, isSubmitting = false, files, selectedFileIds, onClose, onSubmit }: DeleteFilesModalProps) => {
	...
	return (
		<div className={sharedStyles.overlay} aria-label="Close delete images dialog" {...overlayProps}>
			<div
				className={`${sharedStyles.modal} ${styles.deleteFilesModal}`}
				role="dialog"
				aria-modal="true"
			...
			>
				...
				<div className={sharedStyles.warningPanel}>
					...
					{remainingCount > 0 && <p className={styles.remainingNote}>and {remainingCount} more...</p>}
				</div>

				<div className={sharedStyles.actions}>
					<button type="button" className={sharedStyles.cancelButton} onClick={requestClose} disabled={isSubmitting}>
						Cancel
					</button>
					<button
						type="button"
