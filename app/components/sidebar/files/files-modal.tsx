...
import { useFileListPreferences, DEFAULT_FILES_MODAL_PREFERENCES } from '~/hooks/useFileListPreferences';
import {
...
import { ensureCaseConfirmationSummary, type FileConfirmationSummary, type UserConfirmationSummaryDocument } from '~/utils/data';
...
function getItemTypeLabel(summary: FileConfirmationSummary): string {
	const itemTypes = [summary.leftItemType, summary.rightItemType].filter(
		(value): value is NonNullable<FileConfirmationSummary['leftItemType']> => Boolean(value),
	);
...
export const FilesModal = ({
	...
	const { preferences, setSortBy, setConfirmationFilter, setItemTypeFilter, resetPreferences } = useFileListPreferences();
	...
	const existingFileIdSet = useMemo(() => new Set(files.map((file) => file.id)), [files]);
	...
	const paginatedFiles = visibleFiles.slice(effectiveCurrentPage * FILES_PER_PAGE, (effectiveCurrentPage + 1) * FILES_PER_PAGE);

	const shouldForceItemTypeSummaryRefresh = preferences.sortBy === 'itemType' || preferences.itemTypeFilter !== 'all';
	...
	}, [isOpen, currentCase, files, user, confirmationSaveVersion, shouldForceItemTypeSummaryRefresh, initialConfirmationSummary]);
							...
							{visibleFiles.length === 0 ? (
								...
								<ul className={styles.filesList}>
									{paginatedFiles.map((file) => {
										...
										if (summary.includeConfirmation) {
											confirmationClass = summary.isConfirmed ? styles.fileItemConfirmed : styles.fileItemNotConfirmed;
										}
				...
				<div className={styles.footerActions}>
					...
					{totalPages > 1 && (
						<div className={styles.pagination}>
							<button onClick={() => setCurrentPage((previous) => Math.max(0, previous - 1))} disabled={effectiveCurrentPage === 0}>
								Previous
							</button>
							<span>
