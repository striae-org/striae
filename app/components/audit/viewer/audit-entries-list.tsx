...
const isConfirmationImportEntry = (entry: ValidationAuditEntry): boolean => {
	return entry.action === 'confirmation-import' || (entry.action === 'import' && entry.details.workflowPhase === 'confirmation');
};
		...
		<div className={styles.entriesList}>
			...
			{entries.length === 0 ? (
				...
				entries.map((entry) => {
					...
					return (
						<div key={entryKey} className={`${styles.entry} ${styles[entry.result]}`}>
							...
							<div className={styles.entryDetails}>
								...
								{isConfirmationImportEntry(entry) &&
									...
									entry.details.caseDetails.confirmedFileNames.length > 0 && (
										<div className={styles.detailRow}>
											...
											<span className={styles.detailValue}>{entry.details.caseDetails.confirmedFileNames.join(', ')}</span>
										</div>
									)}
									...
									entry.details.fileDetails && (
										<>
											...
											{entry.details.fileDetails.uploadMethod && (
												<div className={styles.detailRow}>
													<span className={styles.detailLabel}>{entry.action === 'file-access' ? 'Access Method' : 'Upload Method'}:</span>
								...
								{(entry.action === 'annotation-create' || entry.action === 'annotation-edit' || entry.action === 'annotation-delete') &&
									entry.details.fileDetails && (
										<>
											{entry.details.fileDetails.fileId && (
												<div className={styles.detailRow}>
								...
								{isExpandable && isExpanded && (
									<div className={styles.expandedDetails}>
										...
										<pre className={styles.expandedDetailsCode}>{JSON.stringify(entry, null, 2)}</pre>
									</div>
								)}
							</div>
						</div>
