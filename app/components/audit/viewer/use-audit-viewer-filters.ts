...
export const useAuditViewerFilters = (caseNumber?: string) => {
	...
	const getFilteredEntries = useCallback(
		(entries: ValidationAuditEntry[]): ValidationAuditEntry[] => {
			...
			return entries.filter((entry) => {
				...
				const reviewerBadgeId = isConfirmationImportEntry(entry) ? entry.details.reviewerBadgeId?.trim().toLowerCase() || '' : '';
				const badgeMatch =
					normalizedBadgeFilter === '' || entryBadgeId.includes(normalizedBadgeFilter) || reviewerBadgeId.includes(normalizedBadgeFilter);
