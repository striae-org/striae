...
export const AuditActivitySummary = ({ caseNumber, filterCaseNumber, dateRangeDisplay, summary }: AuditActivitySummaryProps) => {
	const activeCaseNumber = caseNumber || filterCaseNumber.trim();

	return (
		<div className={styles.summary}>
			<h3>
				{activeCaseNumber ? `Case Activity Summary - ${activeCaseNumber} (${dateRangeDisplay})` : `Activity Summary (${dateRangeDisplay})`}
			</h3>
			<div className={styles.summaryGrid}>
				...
				<div className={styles.summaryItem}>
					...
					<span className={`${styles.value} ${summary.securityIncidents > 0 ? styles.warning : ''}`}>{summary.securityIncidents}</span>
				</div>
			</div>
		</div>
	);
