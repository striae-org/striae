...
const formatDateLabel = (isoDate: string): string => {
	...
	if (Number.isNaN(parsed.getTime()) || parsed.getFullYear() !== year || parsed.getMonth() !== monthIndex || parsed.getDate() !== day) {
		return isoDate;
	}
		...
		<div className={styles.filters}>
					...
					<div className={styles.customDateInputs}>
						...
						<div className={styles.dateRangeButtons}>
							{(customStartDateInput || customEndDateInput) && (
								<button type="button" onClick={onApplyCustomDateRange} className={styles.filterButton} title="Apply custom date range">
									Apply Dates
								</button>
							)}
							{(customStartDate || customEndDate) && (
								<button type="button" onClick={onClearCustomDateRange} className={styles.clearButton} title="Clear custom date range">
									Clear Dates
								</button>
							)}
						</div>
					...
					{!caseNumber && (
						<div className={styles.caseFilterButtons}>
							{caseNumberInput.trim() && (
								<button type="button" onClick={onApplyCaseFilter} className={styles.filterButton} title="Apply case filter">
									Filter
								</button>
							)}
							{filterCaseNumber && (
								<button type="button" onClick={onClearCaseFilter} className={styles.clearButton} title="Clear case filter">
									Clear
								</button>
							)}
						</div>
			...
			<div className={styles.filterGroup}>
				...
				<div className={styles.inputWithButton}>
					...
					<div className={styles.caseFilterButtons}>
						{badgeIdInput.trim() && (
							<button type="button" onClick={onApplyBadgeFilter} className={styles.filterButton} title="Apply badge filter">
								Filter
							</button>
						)}
						{filterBadgeId && (
							<button type="button" onClick={onClearBadgeFilter} className={styles.clearButton} title="Clear badge filter">
								Clear
							</button>
						)}
					</div>
