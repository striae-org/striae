...
const isWorkflowPhase = (phase: unknown): phase is WorkflowPhase =>
	phase === 'casework' || phase === 'case-export' || phase === 'case-import' || phase === 'confirmation' || phase === 'user-management';
...
const buildAuditDateQuery = (dateRange: DateRangeFilter, customStartDate: string, customEndDate: string): AuditDateQuery => {
	...
	const loadAuditData = useCallback(async () => {
		...
		try {
			...
			if (effectiveCaseNumber) {
				...
				const isArchiveBundleCase = Boolean(caseData?.archived === true && caseData?.bundledAuditTrail?.source === 'archive-bundle');
			...
			if (effectiveCaseNumber && entries.length > 0) {
				const trail: AuditTrail = {
					...
					summary: {
						...
						workflowPhases: [...new Set(entries.map((entry) => entry.details.workflowPhase).filter(isWorkflowPhase))],
