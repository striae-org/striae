...
export const UserAuditViewer = ({ isOpen, onClose, caseNumber, title }: UserAuditViewerProps) => {
	...
	const { auditEntries, userData, loading, error, isArchivedReadOnlyCase, bundledAuditWarning, loadAuditData } = useAuditViewerData({
		isOpen,
		user,
		effectiveCaseNumber,
		dateRange,
	...
	const { requestClose, overlayProps } = useOverlayDismiss({
		isOpen,
		onClose,
	});
	...
	}, [effectiveCaseNumber, isExportingPdf, user, userBadgeId, userData?.company, userData?.firstName, userData?.lastName]);

	if (!isOpen) return null;

	return (
		<>
			<Toast message={toastMessage} type={toastType} isVisible={showToast} onClose={() => setShowToast(false)} duration={toastDuration} />
			<div className={styles.overlay} aria-label="Close audit trail dialog" {...overlayProps}>
							...
							<>
								{isArchivedReadOnlyCase && (
									<div className={bundledAuditWarning ? styles.archivedWarning : styles.archivedNotice}>
										<p>{bundledAuditWarning || 'Viewing bundled audit trail data from this imported archived case package.'}</p>
									</div>
								)}

								{/* User Information Section */}
								{user && <AuditUserInfoCard user={user} userData={userData} userBadgeId={userBadgeId} />}
