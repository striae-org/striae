...
export const Navbar = ({
	...
	const caseExportLabel = isArchivedCase ? 'Export Archive' : isReadOnly ? 'Export Confirmations' : 'Export Case Package';
	...
	return (
		<>
			<header className={styles.navbar} aria-label="Canvas top navigation">
				<div className={styles.companyLabelContainer}>
					<div className={styles.companyLabel}>
						{isReviewOnlyCase
							? 'CASE REVIEW ONLY'
							: `${company}${user?.displayName ? ` | ${user.displayName}` : ''}${userBadgeId ? `, ${userBadgeId}` : ''}`}
					</div>
				</div>
						...
						<div className={styles.caseMenuContainer} ref={caseMenuRef}>
							...
							{isCaseMenuOpen && (
								<div className={styles.caseMenu} role="menu" aria-label="Case Management actions">
									...
									{currentCase && <div className={styles.caseMenuCaption}>Case: {currentCase}</div>}
									{archiveDetails?.archived && (
										<div className={styles.caseArchiveDetails}>
											<strong>Archived Case</strong>
											<span>
												Archived At: {archiveDetails.archivedAt ? new Date(archiveDetails.archivedAt).toLocaleString() : 'Unknown'}
											</span>
											<span>
							...
							{isFileMenuOpen && (
								<div className={styles.fileMenu} role="menu" aria-label="Image/File Management actions">
									...
									<button
										...
										title={
											!hasLoadedImage
												? 'Load an image to delete the selected image'
												: isReadOnly
													? 'Cannot delete images for read-only cases'
													: undefined
										}
										onClick={() => {
											onDeleteCurrentFile?.();
									...
									<div className={styles.fileMenuCaption} title={hasLoadedImage && currentFileName ? currentFileName : 'No image loaded'}>
			...
			<CaseImport isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} onImportComplete={onImportComplete} />
			<ManageProfile isOpen={isProfileModalOpen} onClose={() => setIsProfileModalOpen(false)} />
		</>
	);
};
