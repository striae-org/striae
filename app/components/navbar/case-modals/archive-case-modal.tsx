...
export const ArchiveCaseModal = ({ isOpen, currentCase, isSubmitting = false, onClose, onSubmit }: ArchiveCaseModalProps) => {
	...
	return (
		<div className={styles.overlay} aria-label="Close archive case dialog" {...overlayProps}>
			<div className={`${styles.modal} ${styles.modalLarge}`} role="dialog" aria-modal="true" aria-label="Archive Case">
				<button {...getCloseButtonProps({ ariaLabel: 'Close archive case dialog' })}>×</button>
				<h3 className={styles.title}>Archive Case</h3>
				<p className={styles.subtitle}>Case: {currentCase}</p>

				<div className={styles.warningPanel}>
					<p>Archiving a case permanently renders it read-only.</p>
					<p>The archive will be packaged as an encrypted case package and will always include all images.</p>
					<p>The full audit trail is packaged with Striae&apos;s current public key and forensic signatures.</p>
					<p>You can import the archived package back into Striae for future review.</p>
				</div>

				<label htmlFor="archiveReason" className={styles.reasonLabel}>
					Archive reason (recommended)
				</label>
				<textarea
					id="archiveReason"
					ref={reasonRef}
