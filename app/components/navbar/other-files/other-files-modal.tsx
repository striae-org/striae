...
export const OtherFilesModal = ({
	...
	const { overlayProps, getCloseButtonProps, requestClose } = useOverlayDismiss({
		isOpen,
		onClose,
		canDismiss: !isUploading && !isDownloadingSelected && !isDeletingSelected,
	});

	const selectedFiles = useMemo(() => otherFiles.filter((file) => selectedFileIds.has(file.id)), [otherFiles, selectedFileIds]);
			...
			<div className={styles.modal}>
				...
				<div className={styles.modalContent}>
					{(isReadOnly || isReviewOnlyCase) && (
						<p className={styles.readOnlyNotice}>This case is read-only. Upload and deletion are disabled. Download is still allowed.</p>
					)}
					...
					<div className={styles.controls}>
						<p className={styles.countText}>
							{filteredFiles.length} shown of {otherFiles.length} associated file{otherFiles.length === 1 ? '' : 's'}
						</p>
						<div className={styles.bulkSelectionActions}>
							<button type="button" className={styles.secondaryButton} onClick={selectAll} disabled={paginatedFiles.length === 0}>
								Select All Visible
							</button>
							<button type="button" className={styles.secondaryButton} onClick={clearSelected} disabled={selectedFileIds.size === 0}>
								Clear
							</button>
						</div>
					</div>
