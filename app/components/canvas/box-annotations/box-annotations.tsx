...
export const BoxAnnotations = ({
			...
			try {
				...
				if (onAnnotationDataChange && annotationData) {
					onAnnotationDataChange({
						...
						earliestAnnotationTimestamp: resolveEarliestAnnotationTimestamp(annotationData.earliestAnnotationTimestamp, undefined, now),
					});
				}
	...
	const removeBoxAnnotation = useCallback(
		...
		[annotations, onAnnotationsChange, annotationData, onAnnotationDataChange, user, caseNumber, imageFileId, originalImageFileName],
	);
	...
	const handleLabelConfirm = useCallback(async () => {
		...
		try {
			...
			const updatedAnnotations = annotations.map((annotation) =>
				annotation.id === labelDialog.annotationId ? { ...annotation, label: label || undefined } : annotation,
			);
			...
			if (label && presetColorName && annotationData) {
				...
				updatedAdditionalNotes = existingNotes ? `${existingNotes}\n${labelEntry}` : labelEntry;
			}
	...
	const savedAnnotations = useMemo(() => {
		...
		return annotations.map((annotation) => (
			<div
				...
				title={isReadOnly ? undefined : 'Double-click or right-click to delete'}
				onDoubleClick={(e) => {
					if (isReadOnly) return;
					e.stopPropagation();
			...
			>
				{annotation.label && <div className={styles.annotationLabel}>{annotation.label}</div>}
			</div>
		));
			...
			>
				<div className={styles.labelDialogContent}>
					...
					<div className={styles.labelDialogButtons}>
						<button onClick={handleLabelConfirm} className={styles.labelConfirmButton}>
							Confirm
						</button>
						<button onClick={handleLabelCancel} className={styles.labelCancelButton}>
							Cancel
						</button>
					</div>
				</div>
	...
	}, [labelDialog, annotations, handleLabelChange, handleLabelKeyDown, handleLabelConfirm, handleLabelCancel]);
