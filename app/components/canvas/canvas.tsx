...
export const Canvas = ({
	...
	const handleAnnotationDataChange = (data: {
	...
	const handleConfirmation = async (confirmationData: ConfirmationData) => {
		...
		if (user && caseNumber && currentImageId) {
			const success = await storeConfirmation(user, caseNumber, currentImageId, confirmationData, filename, updatedAnnotationData);
	...
	useEffect(() => {
		const hasAnySubclass = annotationData?.leftHasSubclass || annotationData?.rightHasSubclass;
	...
	}, [activeAnnotations, annotationData?.leftHasSubclass, annotationData?.rightHasSubclass, clearFlashingState]);
	...
	return (
		<div className={styles.canvasContainer}>
						...
						<>
							...
							{annotationData?.confirmationData ? (
								<div className={styles.confirmationRow}>
									<div className={styles.confirmationConfirmed}>Identification Confirmed</div>
									<button className={styles.viewConfirmationButton} onClick={() => setIsConfirmationModalOpen(true)}>
										View Confirmation Data
									</button>
								</div>
							) : (
								<div className={styles.confirmationRow}>
									<div className={styles.confirmationIncluded}>{isReadOnly ? 'Confirmation Requested' : 'Confirmation Field Included'}</div>
									{isReadOnly && !isArchivedCase && (
										<button className={styles.confirmButton} onClick={() => setIsConfirmationModalOpen(true)}>
											Confirm
										</button>
									)}
								</div>
					...
					<div className={styles.imageContainer}>
						<div className={styles.imageWrapper}>
							...
							<img
								...
								style={{
									border:
										activeAnnotations?.has('index') && annotationData?.indexType === 'color' && annotationData?.indexColor
							...
							{activeAnnotations?.has('box') && (
								<BoxAnnotations
									...
									annotationData={
										annotationData
											? {
													additionalNotes: annotationData.additionalNotes,
													earliestAnnotationTimestamp: annotationData.earliestAnnotationTimestamp,
												}
											: undefined
									}
							...
							{activeAnnotations?.has('index') && annotationData?.indexType === 'number' && annotationData?.indexNumber && (
								<div className={styles.annotationsOverlay}>
									...
									>
										<div className={styles.caseText}>Index: {annotationData.indexNumber}</div>
									</div>
								</div>
							)}
						</div>
							...
							<aside className={styles.notesPanel} aria-label="Additional notes">
								{annotationData?.leftAdditionalNotes && (
									<div className={styles.noteSection}>
										...
										<div className={styles.additionalNotesBox}>{annotationData.leftAdditionalNotes}</div>
									</div>
								)}
								{annotationData?.rightAdditionalNotes && (
									<div className={styles.noteSection}>
										<div className={styles.notesPanelHeader}>Right Item</div>
										<div className={styles.additionalNotesBox}>{annotationData.rightAdditionalNotes}</div>
									</div>
								)}
								{annotationData?.additionalNotes && (
									<div className={styles.noteSection}>
										<div className={styles.notesPanelHeader}>Notes</div>
										<div className={styles.additionalNotesBox}>{annotationData.additionalNotes}</div>
									</div>
								)}
							</aside>
						)}
			...
			{activeAnnotations?.has('id') && annotationData && annotationData.supportLevel && (
				<div className={styles.supportLevelAnnotation}>
					<div
						...
						style={{
							color: annotationData.supportLevel === 'ID' ? '#28a745' : annotationData.supportLevel === 'Exclusion' ? '#dc3545' : '#ffc107',
						}}
					>
			...
			{activeAnnotations?.has('item') && annotationData && (annotationData.leftHasSubclass || annotationData.rightHasSubclass) && (
				<div className={`${styles.subclassWarning} ${isFlashing ? styles.flashing : ''}`}>
					<div className={styles.subclassText}>POTENTIAL SUBCLASS</div>
				</div>
			)}
