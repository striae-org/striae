...
const buildBaseMetadata = (
  ...
  exportTimestamp: string,
  totalEntries: number
): ConfirmationAuditBundleBaseMetadata => ({
  exportTimestamp,
...
export const verifyConfirmationAuditTrail = async (
  ...
  if (!signatureResult.isValid) {
    throw new Error(
      `Confirmation audit bundle signature verification failed: ${signatureResult.error || 'Unknown signature error'}`
    );
  }
