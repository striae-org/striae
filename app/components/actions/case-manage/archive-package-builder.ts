...
async function fetchImageAsBlob(
	user: User,
	fileData: CaseExportData['files'][number]['fileData'],
	caseNumber: string,
): Promise<Blob | null> {
	try {
...
export async function buildArchivePackage(input: BuildArchivePackageInput): Promise<BuildArchivePackageResult> {
	...
	if (imageFolder && files) {
		for (const fileEntry of files) {
			...
			const exportFileName = generateArchiveImageFilename(fileEntry.fileData.originalFilename, fileEntry.fileData.id);
	...
	const auditEntriesWithExtras = sortAuditEntriesNewestFirst([...auditEntries, ...(auditConfig.additionalEntries ?? [])]);
