...
export const renameCase = async (user: User, oldCaseNumber: string, newCaseNumber: string): Promise<void> => {
	const startTime = Date.now();
	...
	try {
		...
		const endTime = Date.now();
		await auditService.logCaseRename(user, oldCaseNumber, oldCaseNumber, newCaseNumber);

		// Log creation of the new case number as a rename-derived case
		await auditService.logCaseCreation(user, newCaseNumber, newCaseNumber, oldCaseNumber);
...
export const archiveCase = async (user: User, caseNumber: string, archiveReason?: string): Promise<void> => {
	const startTime = Date.now();
	...
	try {
		...
		try {
			...
			const fullName = [userData?.firstName?.trim(), userData?.lastName?.trim()].filter(Boolean).join(' ').trim();
