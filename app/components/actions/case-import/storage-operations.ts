...
import { getUserReadOnlyCases, removeCaseConfirmationSummary, updateUserData, validateUserSession } from '~/utils/data';
import {
	type CaseExportData,
	type FileData,
	type OtherFileData,
 ...
 */
export async function checkReadOnlyCaseExists(user: User, caseNumber: string): Promise<ReadOnlyCaseMetadata | null> {
	try {
 ...
 */
export async function addReadOnlyCaseToUser(user: User, caseMetadata: ReadOnlyCaseMetadata): Promise<void> {
	try {
		...
		if (currentReadOnlyCases.length > 0) {
			...
			console.log(
				`Clearing ${currentReadOnlyCases.length} existing read-only case(s) (${existingCaseNumbers}) before importing new case: ${caseMetadata.caseNumber}`,
			);
		}
...
export async function storeCaseDataInR2(
	...
	try {
		...
		const originalImageIds = originalImageIdMapping ? Object.fromEntries(originalImageIdMapping) : undefined;
		...
		// Store in R2
		const response = await fetchDataApi(user, `/${encodeURIComponent(user.uid)}/${encodeURIComponent(caseNumber)}/data.json`, {
			method: 'PUT',
			headers: {
...
export async function deleteReadOnlyCase(user: User, caseNumber: string): Promise<boolean> {
	...
	try {
		...
		const caseResponse = await fetchDataApi(user, `/${encodeURIComponent(user.uid)}/${encodeURIComponent(caseNumber)}/data.json`, {
			method: 'GET',
		});
		...
		if (caseResponse.status === 404) {
			...
			try {
				...
				if (!removedFromMetadata) {
					console.info(`Read-only case ${caseNumber} metadata was already absent during 404 cleanup.`);
				}
			} catch (removeMetadataError) {
				console.warn(`Failed metadata cleanup for read-only case ${caseNumber} during 404 cleanup:`, removeMetadataError);
			}
		...
		if (caseData.files && caseData.files.length > 0) {
			...
			const failedDeletes = deleteResults.filter(
				(result): result is PromiseRejectedResult => result.status === 'rejected' && !isBenignCleanupError(result.reason),
			);
		...
		if (caseData.otherFiles && caseData.otherFiles.length > 0) {
			const deleteResults = await Promise.allSettled(
				caseData.otherFiles.map((file: OtherFileData) =>
					deleteOtherFile(user, caseNumber, file.id, 'Read-only case clearing - associated files cleanup', {
						skipValidation: true,
						skipCaseDataUpdate: true,
						suppressAudit: true,
					}),
		...
		const deleteCaseResponse = await fetchDataApi(user, `/${encodeURIComponent(user.uid)}/${encodeURIComponent(caseNumber)}/data.json`, {
			method: 'DELETE',
		});
		...
		if (caseDataDeleteHadFailure) {
			console.warn(`Read-only case ${caseNumber} removed from metadata with partial storage cleanup failures.`);
		}
		...
		try {
			...
			if (removedFromMetadata) {
				await removeConfirmationSummary();
				console.warn(
					`Read-only case ${caseNumber} removed from metadata during error fallback. ` + 'Some backing storage may require manual cleanup.',
				);
				return true;
			}
		} catch (removeError) {
