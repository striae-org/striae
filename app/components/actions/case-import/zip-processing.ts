...
function selectPreferredPemPath(pemPaths: string[]): string | undefined {
	...
	const preferred = sortedPaths.find((path) => /^striae-public-signing-key.*\.pem$/i.test(getLeafFileName(path)));

	return preferred ?? sortedPaths[0];
}

async function extractPackagedVerificationPublicKeyFromZip(zip: {
...
async function loadZipArchive(zipFile: File) {
	const JSZip = (await import('jszip')).default;
	const zipSource = typeof zipFile.arrayBuffer === 'function' ? await zipFile.arrayBuffer() : zipFile;

	return JSZip.loadAsync(zipSource);
}
...
export async function previewCaseImport(zipFile: File, currentUser: User): Promise<CaseImportPreview> {
	try {
		...
		if (encryptionManifestFile) {
			...
			try {
				const decryptResult = await decryptExportBatch(currentUser, encryptionManifest, encryptedDataBase64, {});
				decryptedCaseData = JSON.parse(decryptResult.plaintext) as CaseExportData;
			} catch (error) {
				throw new Error(`Failed to decrypt export for preview: ${error instanceof Error ? error.message : 'Unknown error'}`, {
					cause: error,
				});
			}
			...
			if (designatedReviewerEmail) {
				if (!currentUser.email) {
					throw new Error('Unable to verify reviewer designation: your account email is unavailable.');
				}
				if (designatedReviewerEmail.toLowerCase() !== currentUser.email.toLowerCase()) {
					throw new Error('This case package is designated for a specific reviewer. You are not authorized to import this case.');
				}
			}
...
export async function parseImportZip(zipFile: File): Promise<{
	...
	encryptedFiles?: { [filename: string]: string }; // Optional: encrypted files (filename -> base64url)
	isEncrypted?: boolean;
	...
	try {
		...
		const dataFiles = Object.keys(zip.files).filter((name) => name.endsWith('_data.json'));
		...
		const encryptedFiles: { [filename: string]: string } = {};
		...
		try {
			...
			const encryptedFilePromises: Promise<[string, string]>[] = [];
			...
			for (const filePath of fileList) {
				...
				const isBundledAuditFile = filePath === 'audit/case-audit-trail.json' || filePath === 'audit/case-audit-signature.json';
				...
				encryptedFilePromises.push(
					(async () => {
						try {
							...
							throw new Error(`Failed to extract encrypted file ${filePath}: ${err instanceof Error ? err.message : 'Unknown error'}`, {
								cause: err,
							});
						}
					})(),
			...
			const encryptedFileResults = await Promise.all(encryptedFilePromises);
			for (const [filename, data] of encryptedFileResults) {
				encryptedFiles[filename] = data;
			}
		...
		return {
			...
			encryptionManifest,
			encryptedDataBase64,
			encryptedFiles: Object.keys(encryptedFiles).length > 0 ? encryptedFiles : undefined,
			isEncrypted,
			dataFileName,
		};
	} catch (error) {
