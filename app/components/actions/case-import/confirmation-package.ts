...
async function extractConfirmationPackageFromZip(file: File): Promise<ConfirmationImportPackage> {
	...
	if (hasEncryptionManifest) {
		...
		const manifestPath = fileEntries.find((path) => getLeafFileName(path).toLowerCase() === ENCRYPTION_MANIFEST_FILE_NAME);
		if (!manifestPath) {
		...
		const confirmationPaths = fileEntries.filter((path) => CONFIRMATION_EXPORT_FILE_REGEX.test(getLeafFileName(path)));
	...
	const auditTrailPath = fileEntries.find((path) => getLeafFileName(path).toLowerCase() === AUDIT_TRAIL_FILE_NAME);
	const auditManifestPath = fileEntries.find((path) => getLeafFileName(path).toLowerCase() === AUDIT_MANIFEST_FILE_NAME);
...
export async function previewConfirmationImport(file: File, user: User): Promise<ConfirmationImportPreview> {
	const pkg = await extractConfirmationImportPackage(file);

	if (!pkg.isEncrypted || !pkg.encryptedDataBase64) {
		throw new Error('Confirmation imports require an encrypted confirmation ZIP package exported from Striae.');
	}
	...
	try {
		const decryptResult = await decryptExportBatch(user, pkg.encryptionManifest, pkg.encryptedDataBase64, {});
		parsed = JSON.parse(decryptResult.plaintext) as ConfirmationImportData;
	} catch (error) {
		throw new Error(`Failed to decrypt confirmation package for preview: ${error instanceof Error ? error.message : 'Unknown error'}`, {
			cause: error,
		});
	}
