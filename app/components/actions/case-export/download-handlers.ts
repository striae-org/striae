...
function addPublicSigningKeyPemToZip(zip: { file: (path: string, data: string) => unknown }, preferredKeyId?: string): string {
	const preferredPublicKey =
		typeof preferredKeyId === 'string' && preferredKeyId.trim().length > 0 ? getVerificationPublicKey(preferredKeyId) : null;
...
export async function downloadCaseAsZip(
	...
	try {
		...
		if (archivePackageMode) {
			...
			const caseJsonContent = await generateJSONContent(exportData, options.includeUserInfo, false);
...
`;
		...
		const readme = generateZipReadme(exportData, protectForensicData, publicKeyFileName);
...
async function fetchOtherFileAsBlob(user: User, fileData: OtherFileData, caseNumber: string): Promise<Blob | null> {
	try {
