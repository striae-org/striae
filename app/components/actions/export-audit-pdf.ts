...
const buildPartFilename = (caseNumber: string, exportDate: Date, partIndex: number, totalParts: number): string => {
...
export const exportAuditPDF = async ({
	...
	try {
		...
		const isBundledArchivedCase = Boolean(
			caseData?.isReadOnly === true && caseData?.archived === true && caseData?.bundledAuditTrail?.source === 'archive-bundle',
		);
		...
		const sortedEntries = [...allEntries].sort((left, right) => new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime());
