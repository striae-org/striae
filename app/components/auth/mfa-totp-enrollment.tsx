...
export const MfaTotpEnrollment: React.FC<MfaTotpEnrollmentProps> = ({ user, onSuccess, onError, onBack }) => {
	...
	const handleVerify = async () => {
		...
		try {
			...
			try {
				await auditService.logMfaEnrollment(user, undefined, 'totp', 'success', 1, undefined, navigator.userAgent);
			} catch (auditErr) {
		...
		<div className={styles.totpStep}>
			...
			{qrCodeDataUrl && (
				<div className={styles.qrCodeContainer}>
					<p className={styles.note}>Scan this QR code with your authenticator app (Google Authenticator, Authy, etc.).</p>
					<img
						src={qrCodeDataUrl}
			...
			{showSecretKey && totpSecret && (
				<div className={styles.secretKeyDisplay}>
					<p className={styles.note}>If you cannot scan the QR code, enter this key manually in your authenticator app:</p>
					<code className={styles.secretKey}>{totpSecret.secretKey}</code>
				</div>
			)}

			<p className={styles.note}>After scanning, enter the 6-digit code from your authenticator app below.</p>
			<input
				type="text"
				inputMode="numeric"
			...
			<div className={styles.buttonGroup}>
				<button type="button" onClick={handleVerify} disabled={isLoading || verificationCode.length !== 6} className={styles.primaryButton}>
					{isLoading ? 'Verifying…' : 'Complete Setup'}
				</button>

				{onBack && (
					<button type="button" onClick={onBack} disabled={isLoading} className={styles.enrollmentSecondaryButton}>
						Back
					</button>
				)}
			</div>
