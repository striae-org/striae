...
export const MFAVerification = ({ resolver, onSuccess, onError, onCancel }: MFAVerificationProps) => {
	...
	useEffect(() => {
		...
		const hasPhoneHint = resolver.hints.some((h) => h.factorId === PhoneMultiFactorGenerator.FACTOR_ID);
	...
	const verifyTotpCode = async () => {
		...
		try {
			...
			try {
				const sessionId = `session_${result.user.uid}_${Date.now()}_${generateUniqueId(8)}`;
				await auditService.logMfaAuthentication(result.user, 'totp', 'success', 1, sessionId, navigator.userAgent);
			} catch (auditError) {
			...
			<div className={styles.modal}>
				...
				{resolver.hints.length > 1 && (
					<div className={styles.hintSelection}>
						<label htmlFor="hint-select" className={styles.label}>
				...
				{isTotpHint ? (
					<div className={styles.verifyCode}>
						...
						<div className={styles.buttons}>
							<button onClick={verifyTotpCode} disabled={loading || verificationCode.length !== 6} className={styles.button}>
					...
					<>
						{!codeSent ? (
							<div className={styles.sendCode}>
								<p className={styles.description}>We&apos;ll send a verification code to {maskedPhoneNumber}</p>
								<button onClick={sendVerificationCode} disabled={loading} className={styles.button}>
									{loading ? 'Sending...' : 'Send Verification Code'}
								</button>
							</div>
						) : (
							<div className={styles.verifyCode}>
								<p className={styles.description}>Enter the verification code sent to {maskedPhoneNumber}</p>
								<input
									type="text"
									placeholder="Enter 6-digit code"
									value={verificationCode}
								...
								/>
								<div className={styles.buttons}>
									<button onClick={verifyCode} disabled={loading || verificationCode.length !== 6} className={styles.button}>
										{loading ? 'Verifying...' : 'Verify Code'}
									</button>
									<button
										onClick={() => {
