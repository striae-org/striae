...
import { formatPhoneNumberForMfa, getMaskedFactorDisplay, getPhoneDisplayValue, maskPhoneNumber, validatePhoneNumber } from '~/utils/auth';
...
export const MfaPhoneUpdateSection = ({ user, isOpen, onBusyChange }: MfaPhoneUpdateSectionProps) => {
	...
	const handleSendMfaVerificationCode = async () => {
		...
		try {
			...
			if (data?.code === 'auth/requires-recent-login') {
				const supportsPasswordReauth = user.providerData.some((provider) => provider.providerId === 'password');
	...
	const handleVerifyMfaReauthCode = async () => {
		...
		try {
			const credential = PhoneAuthProvider.credential(mfaReauthVerificationId, mfaReauthVerificationCode.trim());
	...
	const handleUpdateMfaPhoneNumber = async () => {
		...
		const formattedPhone = formatPhoneNumberForMfa(mfaPhoneInput);
		const existingPhoneFactorUids = multiFactor(user)
			.enrolledFactors.filter((factor) => factor.factorId === PhoneMultiFactorGenerator.FACTOR_ID)
		...
		try {
			...
			setIsMfaCodeSent(false);
			setMfaResendTimer(0);
			setMfaSuccess(existingPhoneFactorUids.length > 0 ? 'MFA phone number updated successfully.' : 'MFA phone number added successfully.');

			try {
				await auditService.logMfaEnrollment(user, formattedPhone, 'sms', 'success', 1, undefined, navigator.userAgent);
			} catch (auditError) {
			...
			try {
				await auditService.logMfaEnrollment(user, formattedPhone, 'sms', 'failure', undefined, undefined, navigator.userAgent, [
					errorMessage,
				]);
			} catch (auditError) {
			...
			{showMfaReauthPrompt ? (
					...
					{!mfaReauthResolver ? (
						<>
							...
							<div className={styles.mfaButtonGroup}>
								...
								<FormButton variant="secondary" type="button" onClick={handleCancelMfaReauth} disabled={isMfaReauthLoading}>
									Cancel
								</FormButton>
							</div>
						</>
						...
						<>
							<p className={styles.helpText}>
								Password accepted. Send a code to {getMaskedFactorDisplay(mfaReauthHint)} to finish re-authentication.
							</p>
						...
						<>
							...
							<p className={styles.helpText}>Enter the 6-digit code sent to {getMaskedFactorDisplay(mfaReauthHint)}.</p>
							<input
							...
							<div className={styles.mfaButtonGroup}>
								...
								<FormButton variant="secondary" type="button" onClick={handleSendMfaReauthVerificationCode} disabled={isMfaReauthLoading}>
									Send New Code
								</FormButton>
				...
				<div className={styles.mfaVerificationSection}>
					...
					<div className={styles.mfaButtonGroup}>
						...
						<FormButton variant="secondary" type="button" onClick={handleResetMfaChange} disabled={isMfaBusy}>
							Change Phone Number
						</FormButton>

						{mfaResendTimer === 0 ? (
							<FormButton variant="secondary" type="button" onClick={handleSendMfaVerificationCode} disabled={isMfaBusy}>
								Resend Code
							</FormButton>
						) : (
