...
export const DeleteAccount = ({ isOpen, onClose, user, company }: DeleteAccountProps) => {
	...
	const { handleOverlayMouseDown, handleOverlayKeyDown } = useOverlayDismiss({
		isOpen,
		onClose,
	});
	...
	const updateProgress = (totalCases: number, completedCases: number, currentCaseNumber = '', forceComplete = false) => {
		...
		const percent = safeTotal > 0 ? Math.round((safeCompleted / safeTotal) * 100) : forceComplete ? 100 : 0;
				...
				if (eventTypeLine && dataLine) {
					...
					try {
						...
						if (eventType === 'case-start') {
							...
							updateProgress(latestTotalCases, latestCompletedCases, data.currentCaseNumber ?? '');
						}
	...
	return (
		...
		>
			<div className={styles.deleteAccountModal} role="dialog" aria-modal="true" aria-labelledby="modal-title">
				{/* Header */}
				<header className={styles.dangerModalHeader}>
					<h1 id="modal-title" className={styles.dangerTitle}>
						Delete Striae Account
					</h1>
					<button onClick={onClose} className={styles.dangerCloseButton} aria-label="Close modal">
						&times;
					</button>
				</header>
				...
				<div className={styles.modalContent}>
					...
					<div className={styles.warningSection}>
						<p className={styles.warningText}>
							{isDeleting ? (
								...
								<>
									Deleting your account is irreversible! All account information and data will be deleted from Striae. The email address
									associated with this account will be permanently disabled.{' '}
									<strong>
										<em>Please be certain you want to take this action!</em>
									</strong>
								</>
							)}
						</p>
					...
					{showProgress && (
						<div className={styles.progressSection} aria-live="polite">
							...
							>
								<div className={styles.progressFill} style={{ width: `${deletionProgress.percent}%` }} />
							</div>
