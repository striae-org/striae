...
export const Striae = ({ user }: StriaePage) => {
	...
	const { clearSelectedImageState, clearCaseContextState, clearLoadedCaseState } = useStriaeResetHelpers({
		setSelectedImage,
		setSelectedFilename,
		setImageId,
		setAnnotationData,
	...
	const showNotification = (message: string, type: ToastType = 'success', duration = 4000) => {
		setToastType(type);
	...
	const handleDeleteCurrentFileAction = async () => {
		...
		try {
			...
			if (deleteResult.imageMissing) {
				showNotification(`File record deleted. Image asset "${deleteResult.fileName}" was not found and was skipped.`, 'warning');
			} else {
	...
	const handleArchiveCaseSubmit = async (archiveReason: string) => {
		...
		try {
			...
			setArchiveDetails({
				...
				archivedBy: user.uid,
				archivedByDisplay:
					[[userFirstName.trim(), userLastName.trim()].filter(Boolean).join(' ').trim(), userBadgeId.trim()].filter(Boolean).join(', ') ||
					user.uid,
	...
	return (
		<div className={styles.appContainer}>
			...
			/>
			<Toast message={toastMessage} type={toastType} isVisible={showToast} onClose={closeToast} duration={toastDuration} />
		</div>
	);
};
