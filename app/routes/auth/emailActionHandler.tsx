...
export const EmailActionHandler = ({ mode, oobCode, continueUrl, lang }: EmailActionHandlerProps) => {
	...
	const title = mode === 'resetPassword' ? 'Reset Password' : mode === 'verifyEmail' ? 'Verify Email Address' : 'Email Action';
	...
	return (
		<div className={styles.container}>
			<Link viewTransition prefetch="intent" to="https://striae.app" className={styles.logoLink}>
			...
			<div className={styles.formWrapper}>
				...
				{resolvedEmail && <p className={styles.description}>Account: {resolvedEmail}</p>}

				{showLanguageHint && <p className={styles.hint}>This page is currently shown in English.</p>}

				{state === 'loading' && <p className={styles.description}>Validating email action link...</p>}
				...
				{state === 'ready-reset' && (
					<form className={styles.form} onSubmit={handlePasswordResetSubmit}>
						...
						<button type="submit" className={styles.button} disabled={isSubmittingReset}>
				...
				{(state === 'success' || state === 'error') && (
					<div className={styles.actions}>
						{showContinueButton && (
							<button type="button" className={styles.button} onClick={() => navigate(safeContinuePath)}>
								Continue
							</button>
						)}
						<button type="button" className={styles.loginToStriaeButton} onClick={() => navigate('/')}>
							Login to Striae
						</button>
					</div>
				)}
