...
import { type CasesModalPreferences, type CasesModalSortBy, type CasesModalConfirmationFilter } from '~/utils/data/case-filters';
...
function parseStoredPreferences(value: string | null): CasesModalPreferences {
	...
	try {
		...
		const sortBy: CasesModalSortBy =
			parsed.sortBy === 'alphabetical' || parsed.sortBy === 'recent' ? parsed.sortBy : DEFAULT_CASES_MODAL_PREFERENCES.sortBy;
		...
		const showArchivedOnly =
			typeof parsed.showArchivedOnly === 'boolean' ? parsed.showArchivedOnly : DEFAULT_CASES_MODAL_PREFERENCES.showArchivedOnly;
...
export function useCaseListPreferences() {
	const [preferences, setPreferences] = useState<CasesModalPreferences>(() => loadCasesModalPreferences());
	...
	useEffect(() => {
		...
		window.localStorage.setItem(CASES_MODAL_PREFERENCES_STORAGE_KEY, JSON.stringify(preferences));
	}, [preferences]);
