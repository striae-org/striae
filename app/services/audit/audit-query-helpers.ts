import { type AuditQueryParams, type AuditSummary, type ValidationAuditEntry, type WorkflowPhase } from '~/types';

export const applyAuditEntryFilters = (entries: ValidationAuditEntry[], params: AuditQueryParams): ValidationAuditEntry[] => {
...
export const sortAuditEntriesNewestFirst = (entries: ValidationAuditEntry[]): ValidationAuditEntry[] => {
...
export const generateAuditSummary = (entries: ValidationAuditEntry[]): AuditSummary => {
	...
	const phases = [...new Set(entries.map((entry) => entry.details.workflowPhase).filter(Boolean))] as WorkflowPhase[];

	const users = [...new Set(entries.map((entry) => entry.userId))];

	const timestamps = entries.map((entry) => entry.timestamp).sort();
	const securityIncidents = entries.filter(
		(entry) =>
			entry.result === 'failure' &&
			(entry.details.securityChecks?.selfConfirmationPrevented === true || !entry.details.securityChecks?.fileIntegrityValid),
	).length;
