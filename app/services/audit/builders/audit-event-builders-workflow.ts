...
const resolveCaseExportFileType = (fileName: string, exportFormat?: 'json' | 'zip'): AuditFileType => {
	if (exportFormat) {
		switch (exportFormat) {
			case 'json':
				return 'json-data';
...
export const buildCaseExportAuditParams = (input: BuildCaseExportAuditParamsInput): CreateAuditEntryParams => {
...
export const buildCaseImportAuditParams = (input: BuildCaseImportAuditParamsInput): CreateAuditEntryParams => {
	const securityChecks: SecurityCheckResults = {
		selfConfirmationPrevented: input.originalExaminerUid ? input.originalExaminerUid !== input.user.uid : false,
		fileIntegrityValid: input.hashValid,
		exporterUidValidated: input.exporterUidValidated !== undefined ? input.exporterUidValidated : !!input.originalExaminerUid,
		manifestSignaturePresent: input.signatureDetails?.present,
		manifestSignatureValid: input.signatureDetails?.valid,
		manifestSignatureKeyId: input.signatureDetails?.keyId,
	};
...
export const buildConfirmationCreationAuditParams = (input: BuildConfirmationCreationAuditParamsInput): CreateAuditEntryParams => {
	const securityChecks: SecurityCheckResults = {
		selfConfirmationPrevented: false,
	...
	return {
		...
		userProfileDetails: input.badgeId
			...
			: undefined,
		fileDetails:
			input.imageFileId && input.originalImageFileName
				? {
...
export const buildConfirmationExportAuditParams = (input: BuildConfirmationExportAuditParamsInput): CreateAuditEntryParams => {
	const securityChecks: SecurityCheckResults = {
		selfConfirmationPrevented: false,
		fileIntegrityValid: input.result === 'success',
		manifestSignaturePresent: input.signatureDetails?.present,
...
export const buildConfirmationImportAuditParams = (input: BuildConfirmationImportAuditParamsInput): CreateAuditEntryParams => {
	const securityChecks: SecurityCheckResults = {
		selfConfirmationPrevented: input.reviewingExaminerUid ? input.reviewingExaminerUid === input.user.uid : false,
		fileIntegrityValid: input.hashValid,
		exporterUidValidated: input.exporterUidValidated !== undefined ? input.exporterUidValidated : !!input.reviewingExaminerUid,
		manifestSignaturePresent: input.signatureDetails?.present,
		manifestSignatureValid: input.signatureDetails?.valid,
		manifestSignatureKeyId: input.signatureDetails?.keyId,
	};
	...
	return {
		...
		caseDetails:
			input.totalConfirmationsInFile !== undefined
				? {
