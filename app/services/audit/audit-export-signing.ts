...
export const signAuditExport = async (payload: SignAuditExportInput, context: AuditExportContext): Promise<SignedAuditExportPayload> => {
	...
	const caseNumber = context.scopeType === 'case' ? context.caseNumber || context.scopeIdentifier : undefined;

	const signatureResponse = await signAuditExportData(context.user, signatureMetadata, { caseNumber });
