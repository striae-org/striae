...
function hasCaseAccess(userRecord: UserAccessRecord, caseNumber: string): boolean {
  const ownsCase = Array.isArray(userRecord.cases)
    && userRecord.cases.some((entry) => getNonEmptyString(entry?.caseNumber) === caseNumber);

  if (ownsCase) {
    return true;
  }

  return Array.isArray(userRecord.readOnlyCases)
    && userRecord.readOnlyCases.some((entry) => getNonEmptyString(entry?.caseNumber) === caseNumber);
}
...
export async function requireCaseAccess(
  ...
  authenticatedUserId: string,
  caseNumber: unknown
): Promise<ForensicAuthorizationResult> {
