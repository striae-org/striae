...
async function getExportPrivateKeyRegistry(env: Env): Promise<PrivateKeyRegistry> {
	return fetchKeyRegistryFromR2(env.STRIAE_CONFIG, 'export-encryption', env.EXPORT_ENCRYPTION_ACTIVE_KEY_ID, env.REGISTRY_ENCRYPTION_KEY);
}
...
export async function decryptJsonFromStorageWithRegistry(ciphertext: ArrayBuffer, envelope: DataAtRestEnvelope, env: Env): Promise<string> {
...
export async function decryptExportDataWithRegistry(
	...
	for (let index = 0; index < context.candidates.length; index += 1) {
		...
		try {
			const plaintext = await decryptExportData(encryptedDataBase64, wrappedKeyBase64, ivBase64, candidate.privateKeyPem);
			logRegistryDecryptionTelemetry({
				scope: 'export-data',
...
export async function decryptExportImageWithRegistry(
	...
	for (let index = 0; index < context.candidates.length; index += 1) {
		const candidate = context.candidates[index];
		try {
			const imageBlob = await decryptImageBlob(encryptedImageBase64, wrappedKeyBase64, ivBase64, candidate.privateKeyPem);
			logRegistryDecryptionTelemetry({
				scope: 'export-image',
				recordKeyId: context.recordKeyId,
				selectedKeyId: candidate.keyId,
