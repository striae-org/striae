...
export function isValidManifestPayload(
	candidate: Partial<ForensicManifestPayload> & { imageHashes?: unknown },
): candidate is ForensicManifestPayload {
	if (!candidate) {
		return false;
	}
...
export function isValidConfirmationPayload(candidate: Partial<ConfirmationSigningPayload>): candidate is ConfirmationSigningPayload {
	...
	if (typeof metadata.originalExportCreatedAt === 'string' && Number.isNaN(Date.parse(metadata.originalExportCreatedAt))) {
		return false;
	}
...
export function isValidAuditExportPayload(candidate: Partial<AuditExportSigningPayload>): candidate is AuditExportSigningPayload {
	if (!candidate) {
		return false;
	}
...
export function createConfirmationSigningPayload(confirmationData: ConfirmationSigningPayload): string {
	const canonicalPayload = {
		...
		metadata: {
			...
			...(confirmationData.metadata.exportedByBadgeId ? { exportedByBadgeId: confirmationData.metadata.exportedByBadgeId } : {}),
			...
			...(confirmationData.metadata.originalExportCreatedAt
				...
				: {}),
			...(confirmationData.metadata.originalCaseOwnerUid ? { originalCaseOwnerUid: confirmationData.metadata.originalCaseOwnerUid } : {}),
		},
