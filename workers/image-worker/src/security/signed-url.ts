...
export function normalizeSignedUrlTtlSeconds(requestedTtlSeconds: unknown, env: Env): number {
	...
	const fallbackTtl = Number.isFinite(defaultFromEnv) && defaultFromEnv > 0 ? defaultFromEnv : DEFAULT_SIGNED_URL_TTL_SECONDS;
...
export async function verifySignedAccessToken(token: string, fileId: string, env: Env): Promise<boolean> {
	...
	const hmacKey = await getSignedUrlHmacKey(env);
	const signatureValid = await crypto.subtle.verify('HMAC', hmacKey, signatureBuffer, new TextEncoder().encode(payloadBase64Url));
	if (!signatureValid) {
		return false;
	}
