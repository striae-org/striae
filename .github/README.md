...
[![Socket Badge](https://badge.socket.dev/npm/package/@striae-org/striae/10.2.7)](https://badge.socket.dev/npm/package/@striae-org/striae/10.2.7)
...
- **🧽 Scheduled Sweeps and Account Deletion Reliability** - Added a daily scheduled user-worker job that deletes unreferenced `STRIAE_FILES` blobs, plus a durable pending-cleanup marker/retry sweep so account deletion finalizes on cleanup success or a recorded retry marker and blocks outright rather than risk orphaning case data when neither is possible. The marker also now records Firebase Auth deletion as its own durable stage, and the sweep falls back to a live Firebase lookup, so a later KV-deletion or marker-flag failure can no longer strand a marker (and its case data) forever.
- **🛡️ Audit Trail Write Reliability** - The audit write path now retries a failed entry twice more in the background before giving up, deduplicates retried writes by a stable entry ID so a lost response can't create a duplicate entry, and warns the user with a one-time toast only if all attempts fail, without blocking their action.
...
- **🧭 Cross-Case Merge Guardrail** - Added strict signed-scope enforcement for reviewer audit bundle merges so signed bundle scope/case identifiers must match the confirmation target case number before any audit entries are persisted.
