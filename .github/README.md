...
- **🧽 Scheduled Orphan File Sweep** - Added a daily scheduled user-worker job that deletes unreferenced `STRIAE_FILES` blobs, and simplified account deletion so it no longer blocks on incomplete per-case R2 cleanup.
- **🛡️ Audit Trail Write Reliability** - The audit write path now retries a failed entry twice more in the background before giving up, and warns the user with a one-time toast only if all attempts fail, without blocking their action.
