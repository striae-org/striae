...
- **Pending-Cleanup Marker Stall Fix** - Account deletion now records a durable `firebaseAuthDeleted` stage flag immediately after the Firebase Auth user is deleted, before the KV record removal runs, so a later failure in that step (or in flagging full completion) no longer leaves a marker stuck indefinitely. The sweep also falls back to a live Firebase Auth lookup for markers predating this flag, so previously stranded case data becomes eligible for cleanup instead of being skipped forever.
