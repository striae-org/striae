...
export const onRequest = async ({ request, env }: ImageProxyContext): Promise<Response> => {
	...
	if (!proxyPathResult.ok) {
		return proxyPathResult.reason === 'bad-encoding'
			? textResponse('Bad Request: malformed files path encoding', 400)
			: textResponse('Not Found', 404);
	}

	const proxyPath = proxyPathResult.path;

	const envWithFilesWorker = env as Env & { FILES_WORKER?: Fetcher };
	if (!envWithFilesWorker.FILES_WORKER) {
		return textResponse('Files service not configured', 502);
	}
	...
	try {
		upstreamResponse = await envWithFilesWorker.FILES_WORKER.fetch(`https://worker${proxyPath}${requestUrl.search}`, {
			method: request.method,
			headers: upstreamHeaders,
			body: shouldForwardBody ? request.body : undefined,
		});
	} catch {
		return textResponse('Upstream files service unavailable', 502);
	}
