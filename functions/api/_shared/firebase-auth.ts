...
async function verifyTokenSignature(
	...
	const verificationJwk = jwks?.keys?.find((candidate) => candidate.kid === jwtHeader.kid && candidate.kty === 'RSA');
	if (!verificationJwk) {
		return false;
	}
	...
	try {
		return await crypto.subtle.verify({ name: 'RSASSA-PKCS1-v1_5' }, cryptoKey, signatureBytes, signedContent);
	} catch {
		return false;
	}
}
...
export async function verifyFirebaseIdentityFromRequest(request: Request, env: Env): Promise<VerifiedFirebaseIdentity | null> {
	...
	const signatureValid = await verifyTokenSignature(headerSegment, payloadSegment, signatureSegment, jwtHeader);
	if (!signatureValid) {
		return null;
	}
	...
	const uid =
		(typeof jwtPayload.user_id === 'string' && jwtPayload.user_id.trim().length > 0 ? jwtPayload.user_id : jwtPayload.sub) || null;
	if (!uid) {
		return null;
	}
